...
void scan_ports(const char* target_host) {
    ...
    for (int port = 1; port <= 1024; ++port) {
        ...
        if (connect(sockfd, (struct sockaddr*)addr, sizeof(struct sockaddr_in)) == 0) {
            struct servent *srv = getservbyport(htons(port), "tcp");
            const char* service_name = (srv != nullptr) ? srv->s_name : "nieznana";

            std::cout << "[+] Port " << port << " jest OTWARTY | Usługa: " << service_name << std::endl;
            close(sockfd);
        } else {
            std::cout << "[-] Port " << port << " jest ZAMKNIĘTY" << std::endl;
            close(sockfd);
        }
    }
